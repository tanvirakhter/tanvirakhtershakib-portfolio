...
---------------------------------------------------*/
	...
	window.ScrollEffects = function() {
		...
		if ($(window).width() < 1025) {			
			...
			$('.smooth-scroll main, .infinity-lists-holder, .infinity-lists-wrapper, nav, #canvas-slider, #showcase-slider-holder, #showcase-carousel-holder, #hero.has-image, #hero.has-image #hero-styles, #hero-image-wrapper, .thumb-container, .thumb-page').css({'height': winHeight});
		...
		$('#project-nav').each(function(){
			var $this = $(this);			
			var $thisHeight = $('#main').height();
			var $thisTrigger = $('#main-content');
			var $projectNav = $('.next-project-image-wrapper');
			
			var scene = new ScrollMagic.Scene({triggerElement:$thisTrigger[0], duration: $thisHeight})
				.addTo(controller);
...
---------------------------------------------------*/	
	...
	window.FirstLoad = function() {
		...
		if (!$("body").hasClass("project-nav-text")) {
			setTimeout(function(){			
				$(".next-project-image-wrapper").css('background-color', function () {
						return $("#project-nav").data('bgcolor')
					});
			} , 1000 );
		}
		...
		if (!$("body").hasClass("disable-ajaxload")) {
			...
			if ($("#itemsWrapper").hasClass("webgl-fitthumbs")) {
				...
				$('#itemsWrapperLinks .trigger-item-link, #itemsWrapperLinks .trigger-item-link-secondary').on('click', function() {					
					...
					gsap.to('#hero, #show-filters, .item-caption-wrapper, #page-nav, footer', {duration: 0.5, opacity:0, ease:Power4.easeInOut});
			...
			if ($("#itemsWrapper").hasClass("scale-fitthumbs")) {		
				...
				function addListeners(tile, page, url) {
					...
					url.addEventListener("click", function() {				
						...
						gsap.to('#hero, #show-filters, .item, .item-caption-wrapper, #page-nav, footer', {duration: 0.3, opacity: 0, delay:0, ease:Power2.easeIn});
				...
				$('#itemsWrapperLinks .trigger-item-link').on('click', function() {
					...
					gsap.to('#hero, #show-filters, .item, .item-caption-wrapper, #page-nav, footer', {duration: 0.5, opacity:0, ease:Power4.easeInOut});
